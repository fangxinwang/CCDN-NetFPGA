...
void loadcccp (void);
void clearip (void);
void clearipall (void);
void cleararp (void);
void cleararpall (void);
void clearcccp (void);
void clearcccpall (void);
void showq(void);
void setq(void);
...
void help(void) {
  ...
  printf("  listcccp      - Lists entries in CCCP routing table\n");
  ...
  printf("  setcccp       - Set an entry in the CCCP routing table\n");
  ...
  printf("  loadcccp      - Load CCCP routing table entries from a file\n");
  printf("  clearip       - Clear an IP routing table entry\n");
  printf("  clearipall    - Clear all IP routing table entry\n");
  printf("  cleararp      - Clear an ARP table entry\n");
  printf("  cleararpall   - Clear all ARP table entry\n");
  printf("  clearcccp     - Clear an CCCP routing table entry\n");
  printf("  clearcccpal   - Clear all CCCP routing table entry\n");
...
void addcccp(int entry, uint8_t *subnet, int vn, uint8_t *nexthop, int port) {
  writeReg(&nf2, ROUTER_OP_LUT_CCCP_TABLE_ENTRY_NAME_REG, subnet[0] << 24 | subnet[1] << 16 | subnet[2] << 8 | subnet[3]);
  writeReg(&nf2, ROUTER_OP_LUT_CCCP_TABLE_ENTRY_VN_REG, vn);
  writeReg(&nf2, ROUTER_OP_LUT_CCCP_TABLE_ENTRY_NEXT_HOP_IP_REG, nexthop[0] << 24 | nexthop[1] << 16 | nexthop[2] << 8 | nexthop[3]);
  writeReg(&nf2, ROUTER_OP_LUT_CCCP_TABLE_ENTRY_OUTPUT_PORT_REG, port);
  writeReg(&nf2, ROUTER_OP_LUT_CCCP_TABLE_WR_ADDR_REG, entry);
}
...
void setcccp(void) {
  printf("Enter [entry] [name]     [version number]    [nexthop] [port]:\n");
  printf("e.g.     0   192.168.1.0        10            15.1.3.1     4:\n");
  printf(">> ");

  char name[15], nexthop[15];
  int port, entry;
  int vn;
  scanf("%i %s %d %s %x", &entry, name, &vn, nexthop, &port);
  ...
  if ((vn < 0) || (vn > 65535)) {
    printf("Port must be between 0 and ffff(65535).  Aborting\n");
    return;
  }

  uint8_t *sn = parseip(name);
  uint8_t *nh = parseip(nexthop);

  addcccp(entry, sn, vn, nh, port);
}
...
void listcccp(void) {
  int i;
  for (i = 0; i < 32; i++) {
    unsigned name, vn, nh, valport;

    writeReg(&nf2, ROUTER_OP_LUT_CCCP_TABLE_RD_ADDR_REG, i);

    readReg(&nf2, ROUTER_OP_LUT_CCCP_TABLE_ENTRY_NAME_REG, &name);
    
    readReg(&nf2, ROUTER_OP_LUT_CCCP_TABLE_ENTRY_VN_REG, &vn);

    readReg(&nf2,ROUTER_OP_LUT_CCCP_TABLE_ENTRY_NEXT_HOP_IP_REG , &nh);

    readReg(&nf2,ROUTER_OP_LUT_CCCP_TABLE_ENTRY_OUTPUT_PORT_REG , &valport);

    printf("Entry #%i:   ", i);
    int port = valport & 0xff;
    if (name!=0 || port!=0) {
      printf("Name: %i.%i.%i.%i, ", name >> 24, (name >> 16) & 0xff, (name >> 8) & 0xff, name & 0xff);
      printf("Version number: %d, ", vn);
      printf("Next Hop: %i.%i.%i.%i, ", nh >> 24, (nh >> 16) & 0xff, (nh >> 8) & 0xff, nh & 0xff);
      printf("Port: 0x%02x\n", port);
    }
    else {
...
void loadcccp(void) {
  ...
  while (fscanf(fp, "%i %s %d %s %x", &entry, name, &vn, nexthop, &port) != EOF) {
    uint8_t *sn = parseip(name);
    uint8_t *nh = parseip(nexthop);
...
void clearcccp(void) {
  ...
  writeReg(&nf2, ROUTER_OP_LUT_CCCP_TABLE_ENTRY_NAME_REG, 0);
  writeReg(&nf2, ROUTER_OP_LUT_CCCP_TABLE_ENTRY_VN_REG, 0);
  writeReg(&nf2, ROUTER_OP_LUT_CCCP_TABLE_ENTRY_NEXT_HOP_IP_REG, 0);
  writeReg(&nf2, ROUTER_OP_LUT_CCCP_TABLE_ENTRY_OUTPUT_PORT_REG, 0);
  writeReg(&nf2, ROUTER_OP_LUT_CCCP_TABLE_WR_ADDR_REG, entry);
}
...
void clearcccpall(void) {
  ...
  for(i=0; i<32; i++){
    ...
    writeReg(&nf2, ROUTER_OP_LUT_CCCP_TABLE_ENTRY_NEXT_HOP_IP_REG, 0);
    writeReg(&nf2, ROUTER_OP_LUT_CCCP_TABLE_ENTRY_OUTPUT_PORT_REG, 0);
    writeReg(&nf2, ROUTER_OP_LUT_CCCP_TABLE_WR_ADDR_REG, i);
  }
}
...
int parse(char *word) {
  ...
  if (!strcmp(word, "loadmac"))
    return 14;
  if (!strcmp(word, "listcccp"))
    return 15;
  if (!strcmp(word, "setcccp"))
    return 16;
  if (!strcmp(word, "clearcccp"))
    return 17;
  if (!strcmp(word, "loadcccp"))
    return 18;
  if (!strcmp(word, "clearcccpall"))
    return 19;
  if (!strcmp(word, "clearipall"))
    return 20;
  if (!strcmp(word, "cleararpall"))
    return 21;
  if (!strcmp(word, "help"))
    return 8;
  if (!strcmp(word, "quit"))
