...
# --- CCDN
sub make_CCCP_pkt { # len, DA, SA, TTL, DST_IP, SRC_IP, Type, NAME, VN

  my ($len, $DA, $SA, $TTL, $DST_IP, $SRC_IP, $Type, $NAME, $VN) = @_;
  my $cccp_proto = 0xd5;
  ...
  my $CCCP_hdr = NF::CCCP_hdr->new(type => $Type,
								ct_name => $NAME,
								vn => $VN,
								);
  ...
  # create packet filling.... (IP PDU)
  my $PDU = NF::PDU->new($len - $MAC_hdr->length_in_bytes() - $IP_hdr->length_in_bytes() - $CCCP_hdr->length_in_bytes() ); # 这个也要看看，他是payload，看怎么改的，对比着下面的make_ethernet_hdr来看
  my $start_val = $MAC_hdr->length_in_bytes() + $IP_hdr->length_in_bytes() + $CCCP_hdr->length_in_bytes()+1;
  my @data = ($start_val..$len);
  for (@data) {$_ %= 100} # 这里模等于100是不是不想超过100字节？ 因为make_pkt.pl测试脚本里面bytes设置的是98
  $PDU->set_bytes(@data);

  # Return complete packet data
  $MAC_hdr->bytes().$IP_hdr->bytes().$CCCP_hdr->bytes().$PDU->bytes();
}
