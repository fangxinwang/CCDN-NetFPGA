...
sub new   # IP_hdr
  {
    my ($class,%arg) = @_; # 传进来一个class和一个哈希表，class里面到底是什么？？？
    ...
    $IP_hdr->version($arg{'version'}) if (defined $arg{'version'}); # if先执行，而且defined是判断这个值是否空，若空，不执行这句话，前面有默认的定义，arg里面的值是SimLib.pm传进来的参数
...
sub proto
  {
    ...
    return @{$self->{'bytes'}}[9]; # proto 是第9个byte
  }
...
sub dst_ip
  {
    ...
    return Socket::inet_ntoa(pack('N',  # ntoa是将一个串转换成ip地址的格式 ： d.d.d.d， pack（N）是打包成网络长字节
             (@{$self->{'bytes'}}[16]<<24 |
              @{$self->{'bytes'}}[17]<<16 |
              @{$self->{'bytes'}}[18]<<8 |
              @{$self->{'bytes'}}[19]
...
use vars qw(@ISA); # 这句话包含了ISA，表明CCCP_hdr可以继承ISA中的方法
@ISA = qw(NF::PDU); # 这句话把NF::PDU加入到ISA中，意味着CCCP_hdr就可以继承PDU的方法了
...
sub new   # CCCP_hdr
  {
    ...
    my $CCCP_hdr = {
		  'bytes' => [0,    # type, 1 byte, default is 0, means content_request
			      0,	   # identifier, 1 byte, no use
			      0,0,     # checksum, 2 bytes, temporarily no use
			      0,0,0,0, # content_name(ct_name), 4 bytes, default 0
			      0,0,     # version number(vn), 2 bytes, default 0
			     ]
		 };

    bless $CCCP_hdr, $class;

    $CCCP_hdr->type($arg{'type'}) if (defined $arg{'type'}); # if先执行，而且defined是判断这个值是否空，若空，不执行这句话，前面有默认的定义，arg里面的值是SimLib.pm传进来的参数
    $CCCP_hdr->identifier($arg{'identifier'}) if (defined $arg{'identifier'});
    $CCCP_hdr->checksum($arg{'checksum'}) if (defined $arg{'checksum'});
    $CCCP_hdr->ct_name($arg{'ct_name'}) if (defined $arg{'ct_name'});
    $CCCP_hdr->vn($arg{'vn'}) if (defined $arg{'vn'});
...
sub type
  {
    ...
    if (defined $val) {
      my $err = sprintf "CCCP Type is %d (0x%02x) but it must be >= 0 and <= 255", $val,$val;
      if (($val < 0) or ($val > 0xff)) { die "$err" }
      @{$self->{'bytes'}}[0] = $val;
#      $self->checksum(0);
#      $self->checksum($self->calc_checksum);
...
sub identifier
  {
    ...
    if (defined $val) {
      my $err = sprintf "Identifier is %d (0x%02x) but it must be >= 0 and <= 0xff", $val,$val;
      if (($val < 0) or ($val > 0xff)) { die "$err" }
      @{$self->{'bytes'}}[1] = $val;
#      $self->checksum(0);
#      $self->checksum($self->calc_checksum);
...
sub checksum    # Set or retrieve checksum, NOT calulctae it - see below for generation
  {
    ...
    if (defined $val) {
      my $err = sprintf "CCCP Checksum is %d (0x%04x) but it must be >= 0 and <= 0xffff", $val,$val;
      if (($val < 0x0) or ($val > 0xffff)) { die "$err" }
...
# content name is either a 32 bit int or dotted decimal string.
sub ct_name
  {
    my ($self, $val) = @_; # self是什么东西
...
sub vn    # Set or retrieve version number
  {
    my ($self, $val) = @_;

    if (defined $val) {
      my $err = sprintf "CCCP version number is %d (0x%04x) but it must be >= 0 and <= 0xffff", $val,$val;
      if (($val < 0x0) or ($val > 0xffff)) { die "$err" }

      @{$self->{'bytes'}}[8] = int ($val/256);
      @{$self->{'bytes'}}[9] = int ($val%256);
    }
    return (256*@{$self->{'bytes'}}[8] + @{$self->{'bytes'}}[9]);
  }
...
sub get_name {
  my $name = shift;
  if ($name =~ m/^(\d+)\.(\d+)\.(\d+)\.(\d+)$/) {
    my $newname = $1<<24 | $2<<16 | $3<<8 | $4;
    return ($newname,1);
  }
  unless ($name =~ m/^\d+$/) {
    print "Bad content name: $name" ;
    return (0,0);
  }
  return ($name,1);
}
