...
# for hardware files
my %endian_type = ("BIG_ENDIAN"    => '0', # 这是关联数组，索引是big_endian， 值是‘0’
		   "LITTLE_ENDIAN" => '1'
		  );
...
sub nf_packet_in {
  if ($#_ < 4) { confess "nf_packet_in(): Need args: port, length,timeOrDelay,batch,data." } # confess什么意思
  my $port = shift; # shift 是不是指参数的意思
  my $u_len = shift;
...
sub nf_expected_packet {
  ...
  # The actual data is now in @data. May be many strings.
  $data = join ' ',@data;  # 应该是按空格划分,把数组合并成一个变量
  @data = split ' ',$data; # @data是一个数组，包含很多个string
  $len = $#data + 1;
  if (($len < 60) or ($len > 10000)) {
  ...
  foreach (@data) { # 判断data的正则表达，必须是16进制的
...
sub nf_PCI_write32 {
  ...
  if($#_ == 3) { # 这边判断的原因就是传进来的参数可能有$timeOrDelay和$batch也可能没有
    $timeOrDelay = shift;
    $batch = shift;
  }
  else {
